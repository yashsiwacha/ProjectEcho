...
- **Purpose:** Represent a logical isolation boundary for data and configuration.
- **Definition:** An organizational unit or user cohort whose data must be unreachable by another Tenant.
- **Responsibilities:** Provide the scope within which data isolation, configuration overrides, and rate limiting apply.
- **Constraints:** Isolation must be enforced at a named layer (Repository-level guard, row-level security, or schema/database separation) — **this enforcement point is not yet decided** (see Self-Review; this is a security-critical open decision, not a stylistic one). No business component may hardcode tenant-specific logic; tenant variation must be expressed as configuration (see Policy, §31), not branching code.
- **Relationships:** Every Aggregate that is tenant-scoped must carry a Tenant reference as part of its identity. Authorization Policy (§30) is the mechanism that actually checks Tenant boundaries at the point of access.
- **Non-goals:** Does not decide whether ProjectEcho's MVP is single-tenant or multi-tenant — that is a Founder/Product scope decision this document depends on but cannot make.
